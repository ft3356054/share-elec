...
import com.sgcc.uap.share.repositories.BaseProvinceRepository;
 ...
 */
@Service
public class BaseProvinceService implements IBaseProvinceService{
	/** 
     * 注入baseProvinceRepository
     */
	@Autowired
	private BaseProvinceRepository baseProvinceRepository;
	@Autowired
	...
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			baseProvinceRepository.delete(id);
		}
	}
	@Override
	public BaseProvince saveBaseProvince(Map<String,Object> map) throws Exception{
		validateService.validateWithException(BaseProvince.class,map);
		BaseProvince baseProvince = new BaseProvince();
		if (map.containsKey("provinceId")) {
			String provinceId = (String) map.get("provinceId");
			baseProvince = baseProvinceRepository.findOne(provinceId);
			CrudUtils.mapToObject(map, baseProvince,  "provinceId");
		}else{
			CrudUtils.transMap2Bean(map, baseProvince);
		}
		return baseProvinceRepository.save(baseProvince);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<BaseProvince> specification = new Specification<BaseProvince>() {
			@Override
			public Predicate toPredicate(Root<BaseProvince> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<BaseProvince> baseProvince = baseProvinceRepository.findAll(specification,request);
		List<BaseProvince> result = new ArrayList<BaseProvince>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = baseProvince.getContent();
			count = baseProvince.getTotalElements();
		}
