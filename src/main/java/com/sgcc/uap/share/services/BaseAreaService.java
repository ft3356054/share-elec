...
import com.sgcc.uap.share.repositories.BaseAreaRepository;
...
public class BaseAreaService implements IBaseAreaService{
	...
	@Override
	public QueryResultObject getBaseAreaByAreaId(String areaId) {
		BaseArea baseArea = baseAreaRepository.findOne(areaId);
		return RestUtils.wrappQueryResult(baseArea);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			baseAreaRepository.delete(id);
		}
	}
	@Override
	public BaseArea saveBaseArea(Map<String,Object> map) throws Exception{
		validateService.validateWithException(BaseArea.class,map);
		BaseArea baseArea = new BaseArea();
		if (map.containsKey("areaId")) {
			String areaId = (String) map.get("areaId");
			baseArea = baseAreaRepository.findOne(areaId);
			CrudUtils.mapToObject(map, baseArea,  "areaId");
		}else{
			CrudUtils.transMap2Bean(map, baseArea);
		}
		return baseAreaRepository.save(baseArea);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<BaseArea> specification = new Specification<BaseArea>() {
			@Override
			public Predicate toPredicate(Root<BaseArea> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<BaseArea> baseArea = baseAreaRepository.findAll(specification,request);
		List<BaseArea> result = new ArrayList<BaseArea>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = baseArea.getContent();
			count = baseArea.getTotalElements();
		}
