 ...
 */
@Service
public class AuthorityUserService implements IAuthorityUserService{
	/** 
     * 注入authorityUserRepository
     */
	@Autowired
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<AuthorityUser> specification = new Specification<AuthorityUser>() {
			@Override
			public Predicate toPredicate(Root<AuthorityUser> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<AuthorityUser> authorityUser = authorityUserRepository.findAll(specification,request);
		List<AuthorityUser> result = new ArrayList<AuthorityUser>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = authorityUser.getContent();
			count = authorityUser.getTotalElements();
		}
