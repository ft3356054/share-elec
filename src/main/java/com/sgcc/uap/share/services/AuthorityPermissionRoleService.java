...
import com.sgcc.uap.share.repositories.AuthorityPermissionRoleRepository;
 ...
 */
@Service
public class AuthorityPermissionRoleService implements IAuthorityPermissionRoleService{
	/** 
     * 注入authorityPermissionRoleRepository
     */
	@Autowired
	private AuthorityPermissionRoleRepository authorityPermissionRoleRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getAuthorityPermissionRoleById(String id) {
		AuthorityPermissionRole authorityPermissionRole = authorityPermissionRoleRepository.findOne(id);
		return RestUtils.wrappQueryResult(authorityPermissionRole);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			authorityPermissionRoleRepository.delete(id);
		}
	}
	@Override
	public AuthorityPermissionRole saveAuthorityPermissionRole(Map<String,Object> map) throws Exception{
		validateService.validateWithException(AuthorityPermissionRole.class,map);
		AuthorityPermissionRole authorityPermissionRole = new AuthorityPermissionRole();
		if (map.containsKey("id")) {
			String id = (String) map.get("id");
			authorityPermissionRole = authorityPermissionRoleRepository.findOne(id);
			CrudUtils.mapToObject(map, authorityPermissionRole,  "id");
		}else{
			CrudUtils.transMap2Bean(map, authorityPermissionRole);
		}
		return authorityPermissionRoleRepository.save(authorityPermissionRole);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<AuthorityPermissionRole> specification = new Specification<AuthorityPermissionRole>() {
			@Override
			public Predicate toPredicate(Root<AuthorityPermissionRole> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<AuthorityPermissionRole> authorityPermissionRole = authorityPermissionRoleRepository.findAll(specification,request);
		List<AuthorityPermissionRole> result = new ArrayList<AuthorityPermissionRole>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = authorityPermissionRole.getContent();
			count = authorityPermissionRole.getTotalElements();
		}
