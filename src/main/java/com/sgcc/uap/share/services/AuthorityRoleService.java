...
import com.sgcc.uap.share.repositories.AuthorityRoleRepository;
...
public class AuthorityRoleService implements IAuthorityRoleService{
	...
	@Override
	public QueryResultObject getAuthorityRoleById(String id) {
		AuthorityRole authorityRole = authorityRoleRepository.findOne(id);
		return RestUtils.wrappQueryResult(authorityRole);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			authorityRoleRepository.delete(id);
		}
	}
	@Override
	public AuthorityRole saveAuthorityRole(Map<String,Object> map) throws Exception{
		validateService.validateWithException(AuthorityRole.class,map);
		AuthorityRole authorityRole = new AuthorityRole();
		if (map.containsKey("id")) {
			String id = (String) map.get("id");
			authorityRole = authorityRoleRepository.findOne(id);
			CrudUtils.mapToObject(map, authorityRole,  "id");
		}else{
			CrudUtils.transMap2Bean(map, authorityRole);
		}
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<AuthorityRole> specification = new Specification<AuthorityRole>() {
			@Override
			public Predicate toPredicate(Root<AuthorityRole> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<AuthorityRole> authorityRole = authorityRoleRepository.findAll(specification,request);
		List<AuthorityRole> result = new ArrayList<AuthorityRole>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = authorityRole.getContent();
			count = authorityRole.getTotalElements();
		}
