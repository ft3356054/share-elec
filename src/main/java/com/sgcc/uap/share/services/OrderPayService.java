...
import com.sgcc.uap.share.repositories.OrderPayRepository;
...
public class OrderPayService implements IOrderPayService{
	...
	public QueryResultObject getOrderPayByPayId(String payId) {
		OrderPay orderPay = orderPayRepository.findOne(payId);
		return RestUtils.wrappQueryResult(orderPay);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			orderPayRepository.delete(id);
		}
	}
	@Override
	public OrderPay saveOrderPay(Map<String,Object> map) throws Exception{
		validateService.validateWithException(OrderPay.class,map);
		OrderPay orderPay = new OrderPay();
		if (map.containsKey("payId")) {
			String payId = (String) map.get("payId");
			orderPay = orderPayRepository.findOne(payId);
			CrudUtils.mapToObject(map, orderPay,  "payId");
		}else{
			CrudUtils.transMap2Bean(map, orderPay);
		}
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<OrderPay> specification = new Specification<OrderPay>() {
			@Override
			public Predicate toPredicate(Root<OrderPay> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<OrderPay> orderPay = orderPayRepository.findAll(specification,request);
		List<OrderPay> result = new ArrayList<OrderPay>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = orderPay.getContent();
			count = orderPay.getTotalElements();
		}
