 ...
 */
@Service
public class OrderRealAuditService implements IOrderRealAuditService{
	/** 
     * 注入orderRealAuditRepository
     */
	@Autowired
	...
	public QueryResultObject getOrderRealAuditByOrderId(String orderId) {
		OrderRealAudit orderRealAudit = orderRealAuditRepository.findOne(orderId);
		return RestUtils.wrappQueryResult(orderRealAudit);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			orderRealAuditRepository.delete(id);
		}
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<OrderRealAudit> specification = new Specification<OrderRealAudit>() {
			@Override
			public Predicate toPredicate(Root<OrderRealAudit> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<OrderRealAudit> orderRealAudit = orderRealAuditRepository.findAll(specification,request);
		List<OrderRealAudit> result = new ArrayList<OrderRealAudit>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = orderRealAudit.getContent();
			count = orderRealAudit.getTotalElements();
		}
