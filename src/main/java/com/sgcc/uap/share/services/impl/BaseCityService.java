...
import com.sgcc.uap.share.repositories.BaseCityRepository;
 ...
 */
@Service
public class BaseCityService implements IBaseCityService{
	/** 
     * 注入baseCityRepository
     */
	@Autowired
	private BaseCityRepository baseCityRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getBaseCityById(String id) {
		BaseCity baseCity = baseCityRepository.findOne(id);
		return RestUtils.wrappQueryResult(baseCity);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			baseCityRepository.delete(id);
		}
	}
	@Override
	public BaseCity saveBaseCity(Map<String,Object> map) throws Exception{
		validateService.validateWithException(BaseCity.class,map);
		BaseCity baseCity = new BaseCity();
		if (map.containsKey("id")) {
			String id = (String) map.get("id");
			baseCity = baseCityRepository.findOne(id);
			CrudUtils.mapToObject(map, baseCity,  "id");
		}else{
			CrudUtils.transMap2Bean(map, baseCity);
		}
		return baseCityRepository.save(baseCity);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<BaseCity> specification = new Specification<BaseCity>() {
			@Override
			public Predicate toPredicate(Root<BaseCity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<BaseCity> baseCity = baseCityRepository.findAll(specification,request);
		List<BaseCity> result = new ArrayList<BaseCity>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = baseCity.getContent();
			count = baseCity.getTotalElements();
		}
