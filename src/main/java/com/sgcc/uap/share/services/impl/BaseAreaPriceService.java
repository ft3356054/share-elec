 ...
 */
@Service
public class BaseAreaPriceService implements IBaseAreaPriceService{
	/** 
     * 注入baseAreaPriceRepository
     */
	@Autowired
	private BaseAreaPriceRepository baseAreaPriceRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getBaseAreaPriceById(String id) {
	...
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			baseAreaPriceRepository.delete(id);
		}
	}
	@Override
	public BaseAreaPrice saveBaseAreaPrice(Map<String,Object> map) throws Exception{
		validateService.validateWithException(BaseAreaPrice.class,map);
		BaseAreaPrice baseAreaPrice = new BaseAreaPrice();
		if (map.containsKey("id")) {
			String id = (String) map.get("id");
			baseAreaPrice = baseAreaPriceRepository.findOne(id);
			CrudUtils.mapToObject(map, baseAreaPrice,  "id");
		}else{
			CrudUtils.transMap2Bean(map, baseAreaPrice);
		}
		return baseAreaPriceRepository.save(baseAreaPrice);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<BaseAreaPrice> specification = new Specification<BaseAreaPrice>() {
			@Override
			public Predicate toPredicate(Root<BaseAreaPrice> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<BaseAreaPrice> baseAreaPrice = baseAreaPriceRepository.findAll(specification,request);
		List<BaseAreaPrice> result = new ArrayList<BaseAreaPrice>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = baseAreaPrice.getContent();
			count = baseAreaPrice.getTotalElements();
		}
