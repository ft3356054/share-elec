...
import com.sgcc.uap.share.repositories.AuthorityUserRoleRepository;
 ...
 */
@Service
public class AuthorityUserRoleService implements IAuthorityUserRoleService{
	/** 
     * 注入authorityUserRoleRepository
     */
	@Autowired
	...
	@Override
	public QueryResultObject getAuthorityUserRoleById(String id) {
		AuthorityUserRole authorityUserRole = authorityUserRoleRepository.findOne(id);
		return RestUtils.wrappQueryResult(authorityUserRole);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			authorityUserRoleRepository.delete(id);
		}
	}
	@Override
	public AuthorityUserRole saveAuthorityUserRole(Map<String,Object> map) throws Exception{
		validateService.validateWithException(AuthorityUserRole.class,map);
		AuthorityUserRole authorityUserRole = new AuthorityUserRole();
		if (map.containsKey("id")) {
			String id = (String) map.get("id");
			authorityUserRole = authorityUserRoleRepository.findOne(id);
			CrudUtils.mapToObject(map, authorityUserRole,  "id");
		}else{
			CrudUtils.transMap2Bean(map, authorityUserRole);
		}
		return authorityUserRoleRepository.save(authorityUserRole);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<AuthorityUserRole> specification = new Specification<AuthorityUserRole>() {
			@Override
			public Predicate toPredicate(Root<AuthorityUserRole> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<AuthorityUserRole> authorityUserRole = authorityUserRoleRepository.findAll(specification,request);
		List<AuthorityUserRole> result = new ArrayList<AuthorityUserRole>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = authorityUserRole.getContent();
			count = authorityUserRole.getTotalElements();
		}
