 ...
 */
@Service
public class NotifyAnnounceUserService implements INotifyAnnounceUserService{
	/** 
     * 注入notifyAnnounceUserRepository
     */
	@Autowired
	...
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			notifyAnnounceUserRepository.delete(id);
		}
	}
	@Override
	public NotifyAnnounceUser saveNotifyAnnounceUser(Map<String,Object> map) throws Exception{
		...
		return notifyAnnounceUserRepository.save(notifyAnnounceUser);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<NotifyAnnounceUser> specification = new Specification<NotifyAnnounceUser>() {
			@Override
			public Predicate toPredicate(Root<NotifyAnnounceUser> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<NotifyAnnounceUser> notifyAnnounceUser = notifyAnnounceUserRepository.findAll(specification,request);
		List<NotifyAnnounceUser> result = new ArrayList<NotifyAnnounceUser>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = notifyAnnounceUser.getContent();
			count = notifyAnnounceUser.getTotalElements();
		}
