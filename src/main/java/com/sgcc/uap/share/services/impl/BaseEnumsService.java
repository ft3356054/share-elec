...
import com.sgcc.uap.share.repositories.BaseEnumsRepository;
 ...
 */
@Service
public class BaseEnumsService implements IBaseEnumsService{
	/** 
     * 注入baseEnumsRepository
     */
	@Autowired
	private BaseEnumsRepository baseEnumsRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getBaseEnumsByEnumsId(String enumsId) {
		BaseEnums baseEnums = baseEnumsRepository.findOne(enumsId);
		return RestUtils.wrappQueryResult(baseEnums);
	}
	@Override
	@Cacheable(cacheNames = "baseEnums" ,  keyGenerator = "wiselyKeyGenerator") //redis缓存
	public BaseEnums getBaseEnumsByTypeAndStatus(String enumsType,String enumsStatus) {
		BaseEnums baseEnums = baseEnumsRepository.findByEnumsTypeAndEnumsStatus(enumsType,enumsStatus);
		return baseEnums;
	}
	
	@Override
	@CacheEvict(cacheNames = "baseEnums",keyGenerator = "wiselyKeyGenerator" , allEntries = true) //redis缓存
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			baseEnumsRepository.delete(id);
		}
	}
	@Override
	@CacheEvict(cacheNames = "baseEnums",keyGenerator = "wiselyKeyGenerator" , allEntries = true) //redis缓存
	public BaseEnums saveBaseEnums(Map<String,Object> map) throws Exception{
		validateService.validateWithException(BaseEnums.class,map);
		BaseEnums baseEnums = new BaseEnums();
		if (map.containsKey("enumsId")) {
			String enumsId = (String) map.get("enumsId");
			baseEnums = baseEnumsRepository.findOne(enumsId);
			CrudUtils.mapToObject(map, baseEnums,  "enumsId");
		}else{
			CrudUtils.transMap2Bean(map, baseEnums);
		}
		return baseEnumsRepository.save(baseEnums);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<BaseEnums> specification = new Specification<BaseEnums>() {
			@Override
			public Predicate toPredicate(Root<BaseEnums> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<BaseEnums> baseEnums = baseEnumsRepository.findAll(specification,request);
		List<BaseEnums> result = new ArrayList<BaseEnums>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = baseEnums.getContent();
			count = baseEnums.getTotalElements();
		}
