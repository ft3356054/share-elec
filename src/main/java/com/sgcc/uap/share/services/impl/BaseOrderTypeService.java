 ...
 */
@Service
public class BaseOrderTypeService implements IBaseOrderTypeService{
	/** 
     * 注入baseOrderTypeRepository
     */
	@Autowired
	...
	@Override
	public QueryResultObject getBaseOrderTypeByOrderTypeId(String orderTypeId) {
		BaseOrderType baseOrderType = baseOrderTypeRepository.findOne(orderTypeId);
		return RestUtils.wrappQueryResult(baseOrderType);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			baseOrderTypeRepository.delete(id);
		}
	}
	@Override
	public BaseOrderType saveBaseOrderType(Map<String,Object> map) throws Exception{
		validateService.validateWithException(BaseOrderType.class,map);
		BaseOrderType baseOrderType = new BaseOrderType();
		if (map.containsKey("orderTypeId")) {
			String orderTypeId = (String) map.get("orderTypeId");
			baseOrderType = baseOrderTypeRepository.findOne(orderTypeId);
			CrudUtils.mapToObject(map, baseOrderType,  "orderTypeId");
		}else{
			CrudUtils.transMap2Bean(map, baseOrderType);
		}
		return baseOrderTypeRepository.save(baseOrderType);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<BaseOrderType> specification = new Specification<BaseOrderType>() {
			@Override
			public Predicate toPredicate(Root<BaseOrderType> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<BaseOrderType> baseOrderType = baseOrderTypeRepository.findAll(specification,request);
		List<BaseOrderType> result = new ArrayList<BaseOrderType>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = baseOrderType.getContent();
			count = baseOrderType.getTotalElements();
		}
