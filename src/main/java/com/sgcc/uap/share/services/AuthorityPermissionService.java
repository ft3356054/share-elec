...
import com.sgcc.uap.share.repositories.AuthorityPermissionRepository;
 ...
 */
@Service
public class AuthorityPermissionService implements IAuthorityPermissionService{
	/** 
     * 注入authorityPermissionRepository
     */
	@Autowired
	...
	public QueryResultObject getAuthorityPermissionById(String id) {
		AuthorityPermission authorityPermission = authorityPermissionRepository.findOne(id);
		return RestUtils.wrappQueryResult(authorityPermission);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			authorityPermissionRepository.delete(id);
		}
	}
	@Override
	public AuthorityPermission saveAuthorityPermission(Map<String,Object> map) throws Exception{
		validateService.validateWithException(AuthorityPermission.class,map);
		AuthorityPermission authorityPermission = new AuthorityPermission();
		if (map.containsKey("id")) {
			String id = (String) map.get("id");
			authorityPermission = authorityPermissionRepository.findOne(id);
			CrudUtils.mapToObject(map, authorityPermission,  "id");
		}else{
			CrudUtils.transMap2Bean(map, authorityPermission);
		}
		return authorityPermissionRepository.save(authorityPermission);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<AuthorityPermission> specification = new Specification<AuthorityPermission>() {
			@Override
			public Predicate toPredicate(Root<AuthorityPermission> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<AuthorityPermission> authorityPermission = authorityPermissionRepository.findAll(specification,request);
		List<AuthorityPermission> result = new ArrayList<AuthorityPermission>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = authorityPermission.getContent();
			count = authorityPermission.getTotalElements();
		}
