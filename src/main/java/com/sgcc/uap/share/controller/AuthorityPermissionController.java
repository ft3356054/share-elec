...
import com.sgcc.uap.share.services.IAuthorityPermissionService;
import com.sgcc.uap.share.vo.AuthorityPermissionVO;
...
public class AuthorityPermissionController {
     ...
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(AuthorityPermissionController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = authorityPermissionService.getAuthorityPermissionById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			authorityPermissionService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(authorityPermissionService.saveAuthorityPermission(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = authorityPermissionService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, AuthorityPermissionVO.class);
