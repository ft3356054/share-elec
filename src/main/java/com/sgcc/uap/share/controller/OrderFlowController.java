...
import com.sgcc.uap.share.services.IOrderFlowService;
import com.sgcc.uap.share.vo.OrderFlowVO;
...
public class OrderFlowController {
     ...
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(OrderFlowController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	/**
	 * @getByFlowId:根据flowId查询
	 * @param flowId
	 * @return WrappedResult 查询结果
	 * @date 2020-11-26 14:32:47
	 * @author 18511
	 */
	@RequestMapping(value = "/{flowId}")
	public WrappedResult getByFlowId(@PathVariable String flowId) {
		try {
			QueryResultObject result = orderFlowService.getOrderFlowByFlowId(flowId);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			orderFlowService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(orderFlowService.saveOrderFlow(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = orderFlowService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, OrderFlowVO.class);
