...
import com.sgcc.uap.share.services.IAuthorityPermissionRoleService;
import com.sgcc.uap.share.vo.AuthorityPermissionRoleVO;
...
@RestController
@Transactional
@RequestMapping("/authorityPermissionRole")
public class AuthorityPermissionRoleController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(AuthorityPermissionRoleController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	/** 
     * AuthorityPermissionRole服务
     */
	@Autowired
	private IAuthorityPermissionRoleService authorityPermissionRoleService;
	/**
	 * @getById:根据id查询
	 * @param id
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = authorityPermissionRoleService.getAuthorityPermissionRoleById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			authorityPermissionRoleService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(authorityPermissionRoleService.saveAuthorityPermissionRole(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = authorityPermissionRoleService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, AuthorityPermissionRoleVO.class);
