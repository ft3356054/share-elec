...
@RestController
@Transactional
@RequestMapping("/elecPosition")
public class ElecPositionController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(ElecPositionController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			elecPositionService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(elecPositionService.saveElecPosition(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = elecPositionService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, ElecPositionVO.class);
