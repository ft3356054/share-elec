 ...
 */
@Service
public class ElectricianStatusService implements IElectricianStatusService{
	/** 
     * 注入electricianStatusRepository
     */
	@Autowired
	private ElectricianStatusRepository electricianStatusRepository;
	@Autowired
	...
	public QueryResultObject getElectricianStatusByElectricianId(String electricianId) {
		...
		return RestUtils.wrappQueryResult(electricianStatus);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			electricianStatusRepository.delete(id);
		}
	}
	@Override
	public ElectricianStatus saveElectricianStatus(Map<String,Object> map) throws Exception{
		validateService.validateWithException(ElectricianStatus.class,map);
		...
		if (map.containsKey("electricianId")) {
			...
			electricianStatus = electricianStatusRepository.findOne(electricianId);
			CrudUtils.mapToObject(map, electricianStatus,  "electricianId");
		}else{
			CrudUtils.transMap2Bean(map, electricianStatus);
		}
		return electricianStatusRepository.save(electricianStatus);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<ElectricianStatus> specification = new Specification<ElectricianStatus>() {
			@Override
			public Predicate toPredicate(Root<ElectricianStatus> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<ElectricianStatus> electricianStatus = electricianStatusRepository.findAll(specification,request);
		List<ElectricianStatus> result = new ArrayList<ElectricianStatus>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = electricianStatus.getContent();
			count = electricianStatus.getTotalElements();
		}
