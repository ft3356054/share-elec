...
import com.sgcc.uap.share.electrician.vo.ElectricianStatusVO;
...
@RestController
@Transactional
@RequestMapping("/electricianStatus")
public class ElectricianStatusController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(ElectricianStatusController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
     ...
     */
	@Autowired
	private IElectricianStatusService electricianStatusService;
	/**
	...
	public WrappedResult getByElectricianId(@PathVariable String electricianId) {
		try {
			QueryResultObject result = electricianStatusService.getElectricianStatusByElectricianId(electricianId);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			electricianStatusService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(electricianStatusService.saveElectricianStatus(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = electricianStatusService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, ElectricianStatusVO.class);
