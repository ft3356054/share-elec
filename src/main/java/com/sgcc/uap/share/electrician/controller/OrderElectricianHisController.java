...
import com.sgcc.uap.share.electrician.vo.OrderElectricianHisVO;
...
@RestController
@Transactional
@RequestMapping("/orderElectricianHis")
public class OrderElectricianHisController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(OrderElectricianHisController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	/** 
     * OrderElectricianHis服务
     */
	@Autowired
	private IOrderElectricianHisService orderElectricianHisService;
	/**
	...
	public WrappedResult getByOrderElectricianId(@PathVariable String orderElectricianId) {
		try {
			QueryResultObject result = orderElectricianHisService.getOrderElectricianHisByOrderElectricianId(orderElectricianId);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			orderElectricianHisService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(orderElectricianHisService.saveOrderElectricianHis(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = orderElectricianHisService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, OrderElectricianHisVO.class);
